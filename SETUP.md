...
- **初回実行**: 監視チャンネルの**最新1本**だけ生成します（音声・記事・ページ・フィード・LINE通知）。それより前の既存動画はさかのぼらず「処理済み(seed)」として記録するだけです。これは正常な動作です。
- **2回目以降**: 初回以降に投稿された**新着動画があれば**、音声・記事・ページ・フィードが生成され、LINEに通知が届きます。
- 生成後は `docs/` 配下が自動コミットされ、GitHub Pages に反映されます。
- 無料枠保護のため**1実行あたり最大 `generation.max_per_run` 本**（既定3）まで。多数たまっている場合は6時間ごとの実行で順次消化されます。
...
| 症状 | 原因・対処 |
|------|-----------|
| 手動実行したのに音声/記事が出ない | 初回は**最新1本**を生成します。出ない場合は `GEMINI_API_KEY` 未設定か、最新が視聴不可（非公開等）かを確認。以降は新着が投稿されてから生成されます。 |
